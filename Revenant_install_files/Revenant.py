...
class cipher:
    ...
    def encrypt(self, path=str) -> int:
        ...
        if os.path.isfile(path) is True:
            ...
            for object_ in objects:
                del object_
            self.log("s", path, encrypted=True)
            return 0
        elif os.path.isdir(path) is True:
            for root, dirs, files in os.walk(path):
                for file in files:
                    ...
                    for object_ in objects:
                        del object_
            self.log("f", path, encrypted=True)
            return 0
        else:
    ...
    def decrypt(self, path=str) -> int:
        ...
        """
        return_code = self.guard_clause(path, decrypting=True)
        if return_code != 0:
            return return_code
        if os.path.isfile(path) is True:
            with open (path, "rb") as file:
            ...
            for object_ in objects:
                del object_
            self.log("s", path, encrypted=False)
            return 0
        elif os.path.isdir(path) is True:
            for root, dirs, files in os.walk(path):
                for file in files:
                    ...
                    for object_ in objects:
                        del object_
            self.log("f", path, encrypted=False)
            return 0    
        else:
            sg.popup_auto_close("WHAT DID YOU DO????? the app didn't detect the file as either a file or a dir. what? email me pls", font="Helvetica")    
            ...
            try:
                for root, dirs, files in os.walk("/home/{}/Vault".format(self.username)):
                    for file in files:
                        ...
                        with open (file_name, "wb") as file:
                            file.write(original_data)
                self.log("v", path=" ", encrypted=False)
                return 0
            except FileNotFoundError:
                try:
                ...
                for root, dirs, files in os.walk("/home/{}/Vault".format(self.username)):
                    for file in files:
                        ...
                        with open (file_name, "wb") as file:
                            ...
                            file.write(key_signature)
                            file.write(ciphered_data)
                self.log("v", path=" ", encrypted=True)
                return 0                        
            except FileNotFoundError:
                try:
                    os.mkdir("/home/{}/Vault".format(self.username))
...
sg.theme('DarkBlue')
# Graphical layouts to simulate an interactive window
login_layout = [
